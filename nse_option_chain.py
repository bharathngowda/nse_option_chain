...
option_chain_final=pd.DataFrame()
option_chain=pd.DataFrame()

# Symbols
symbols=['NIFTY','NIFTYNXT50','FINNIFTY','BANKNIFTY','MIDCPNIFTY']

for i in symbols:
    # URL of the API endpoint
    api_url = "https://www.nseindia.com/api/option-chain-indices?symbol=" + i
    ...
    while attempt<10:
        try:
            ...
            try:
                ...
                option_chain['symbol']=i
                
                # Combining all symbols
                option_chain_final=pd.concat([option_chain_final,option_chain])
...
# Exporting to excel
if option_chain_final.empty==False:
    option_chain_final.to_excel('data/option_chain_' + datetime.now().strftime('%Y%m%d%H%M%S') + '.xlsx')
